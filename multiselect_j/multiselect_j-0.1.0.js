...
(function ($) {
  const PLUGIN_NAME = "multiselect_j";
  const DEFAULTS = {
    height: "7rem",
    width: "100%",
  };
  ...
  */
  ...
  function Multiselect(elem, opts) {
    ...
    this.msj_btn = $("<button/>")
      .text("Not Selected")
      .css({ "max-height": this.opts.height, width: this.opts.width });
    ...
    this.msj_opt.append(
      $("<div/>", { class: "msj-search" }).append(
        $("<input/>", { type: "text", placeholder: "Search Keyword" }).on(
          "change",
          function (e, v) {
            var val = $(this).val();
    ...
    this.elem.on("change", (ev) => {
      this.elem.val().forEach((it) => {
        ...
        if (found.length && !found.prop("checked"))
          found.prop("checked", true).trigger("change");
      });
    });
  }
  ...
  function multiselect_add(self, text, value) {
    ...
    self.msj_opt.append(
      $("<div/>", { class: "msj-option" }).append(
        $("<label/>", { for: opt_label })
          .append(
            $("<input/>", { id: opt_label, type: "checkbox", value }).on(
              "change",
              function () {
                self.elem
                ...
                var checkedVal = self.msj_opt
                  ...
                  .toArray();
                self.msj_btn.text(
                  checkedVal.length > 0 ? checkedVal.join(",") : "Not Selected"
                );
              }
            )
          )
  ...
  $.fn[PLUGIN_NAME] = function (opts) {
    ...
    if (undefined == opts || "object" === typeof opts) {
      return this.each(function () {
        ...
        $.data(this, PLUGIN_NAME, new Multiselect(this, opts));
      });
